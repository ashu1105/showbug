...
<nav class="navbar navbar-inverse navbar-fixed-top">
      <div class="container-fluid">
        <ul class="nav navbar-nav">
         <li><a href="movies1.php?uid=<?php echo $uid;?>">MOVIES</a></li>
         <li class="active"><a href="plays1.php?uid=<?php echo $uid;?>">PLAYS</a></li>
       </ul>
...
<div class="carousel slide" id="featured"  data-ride="carousel" data-interval="2000">
  ...
  <div class="carousel-inner" style="position:relative;margin-top: 50px;" >
    <div class="item active" >
     <?php 
     $query = "SELECT * FROM `Shows` WHERE Release_Date < '2017-09-01' and Movie_type = 'Play' ORDER BY Release_Date DESC LIMIT 3 ";   
     $result = $con->query($query);
     $result->data_seek(0);  
     $i=1;
  ...
  <div class="item">
    ...
    <form action="" method="POST" style="margin-left: 20%; position: absolute;top: 20px;width:60%; "> 
      <div class="input-group">
       <input type="text" class="form-control" style="height: 50px;" placeholder="   Search Plays Here" name="Search" id="txtSearch"/>
...
{
  $query = "CALL patternmatch('".$pattern."','Play')";
  $result = $con->query($query);
  $result->data_seek(0);  
  $i=1;
  while($row = $result->fetch_assoc()){       
...
<h1 class="well well-sm" style="text-align: center;"> Plays</h1>
<ul class="nav nav-tabs">
  <form  action="" method="POST">
    <button name = "Hindi" value = "Hindi"  >Hindi</button>
    <button name = "Marathi" value = "Marathi"  >Marathi</button>
  </form>
</ul>
<?php  
...
$result = $con->query($query);
if($result->num_rows ==0){echo "<br><center>No plays to display!</center>";}
$result->data_seek(0);  
$i=1;
while($row = $result->fetch_assoc()){       
  $address[$i] = $row[Poster1];
...
<div style="margin:20px;">
  ...
  <div class="row"  >
    ...
    <div class="col-md-3">
      <div class="square2">
        <div class="square">
          <img src=<?php echo $address[$b]; ?> alt="Hitman's Bodyguard"   class="blockimages">
          <div class="overlay">
            <a href="info.php?variable=<?php echo $ID[$b]; ?>&uid=<?php echo $uid; ?>" class="info"><span class="glyphicon glyphicon-info-sign"></span> Info</a>
          </div>
          <div class="details">
